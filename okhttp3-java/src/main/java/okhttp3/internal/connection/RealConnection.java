...
import static okhttp3.OkHttpClient.TAG;
...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    private static final int MAX_TUNNEL_ATTEMPTS = 21; // max tunnel attempts
    ...
    private void connectSocket(int connectTimeout, int readTimeout, Call call, EventListener eventListener) throws IOException {
        ...
        Log.d(TAG, "connectSocket: hostName=" + route.socketAddress().getHostName() + ", Address=" + route.socketAddress().getAddress());
        ...
        try {
            // TODO 为输入输出流赋值
            source = Okio.buffer(Okio.source(rawSocket)); // 输入 input
            sink = Okio.buffer(Okio.sink(rawSocket)); // 输出output
