...
VideoGLRender::VideoGLRender() : VideoRender(VIDEO_RENDER_OPENGL) {
    LOGD("VideoGLRender::VideoGLRender()");
}
...
void VideoGLRender::Init(int videoWidth, int videoHeight, int *dstSize) {
    LOGD("VideoGLRender::InitRender video[w, h]=[%d, %d]", videoWidth, videoHeight);
    if (dstSize != nullptr) {
...
void VideoGLRender::OnSurfaceChanged(int w, int h) {
    LOGE("VideoGLRender::OnSurfaceChanged [w, h]=[%d, %d]", w, h);
    m_ScreenSize.x = w;
    m_ScreenSize.y = h;
    glViewport(0, 0, w, h);
...
void VideoGLRender::RenderVideoFrame(NativeImage *pImage) {
    LOGD("VideoGLRender::RenderVideoFrame pImage=%p", pImage);
...
void VideoGLRender::OnDrawFrame() {
    ...
    LOGE("VideoGLRender::OnDrawFrame [w, h]=[%d, %d], format=%d", m_RenderImage.width,
...
void VideoGLRender::UpdateMVPMatrix(TransformMatrix *pTransformMatrix) {
    ...
    LOGD("VideoGLRender::UpdateMVPMatrix rotate %d,%.2f,%0.5f,%0.5f,%0.5f,%0.5f,",
